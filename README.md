...
- **되돌리기 / 다시** — 상단 **← / →** 버튼 또는 `Ctrl+Z` / `Ctrl+Shift+Z`. HWP처럼 **어절 묶음 단위**로 되돌립니다. (글자수 표기는 이 버튼으로 대체)
- **‘전환’ 버튼** — 블록 손잡이의 추가·설정 옆 **전환** 버튼으로 텍스트·제목·목록·체크리스트·코드·토글·콜아웃 등 **여러 블록을 한꺼번에** 양식 변경(전환 가능한 것만, 목록↔텍스트는 분할/병합). 메뉴 위 **검색창**으로 빠르게 고릅니다. *(v6i: 기존 설정의 ‘변환’·텍스트 선택 ‘Convert to’를 전환으로 통합, 드래그로 여러 블록 선택 시 일괄 전환이 확실히 동작하도록 수정.)*
- **자동 변환** — 입력 중 `->`→**→**, `<-`→**←**, `=>`→**⇒**, `...`→**…**, `(c)`→**©** 등.
