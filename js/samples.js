...
const Samples = (() => {
  ...
  const USAGE_ICON = '📖';
  const USAGE_SEARCH =
    '사용법 도움말 가이드 매뉴얼 시작 글쓰기 서식 굵게 링크 되돌리기 다시 실행 undo redo ctrl z 자동변환 화살표 전환 변환 일괄 블록 추가 슬래시 목록 코드 표 함수 수식 좌표 천단위 콤마 sum average max min count product 사칙연산 파일 첨부 pdf word excel hwp 인용구 토글 콜아웃 이미지 북마크 하위문서 목차 드래그 순서 이동 양방향 동기화 복제 내보내기 백업 markdown 저장 자동저장 단축키 페이지 새 하위 이름 삭제 즐겨찾기 아이콘 커버 검색 구글 드라이브 연결 client id 배포 로그인 유지 데모';
  ...
  function _blocks() {
    return [
      ...
      { type: 'list', data: { style: 'unordered', items: [
        { content: '<b>되돌리기 / 다시</b> — 상단의 <b>← / →</b> 버튼 또는 <code class="inline-code">Ctrl+Z</code> / <code class="inline-code">Ctrl+Shift+Z</code> (한 번에 한 묶음씩)', items: [] },
        { content: '<b>자동 변환</b> — 입력 중 <code class="inline-code">-&gt;</code>→<b>→</b>, <code class="inline-code">&lt;-</code>→<b>←</b>, <code class="inline-code">=&gt;</code>→<b>⇒</b>, <code class="inline-code">...</code>→<b>…</b>, <code class="inline-code">(c)</code>→<b>©</b>', items: [] },
        { content: '<b>전환</b> — 블록 손잡이의 <b>전환</b> 버튼(추가·설정 옆). 여러 블록을 범위로 잡아 한꺼번에 양식 바꾸기(아래 참고)', items: [] },
      ] } },

      { type: 'header', data: { text: '🔁 2. 블록 양식 전환 (여러 개 한 번에)', level: 2 } },
      { type: 'paragraph', data: { text: '블록에 마우스를 올리면 왼쪽 손잡이에 <b>추가(+) · 설정(⠿) · 전환</b> 버튼이 있어요. <b>전환</b>을 누르면 텍스트·제목·목록·체크리스트·인용구·코드·토글·콜아웃으로 바꿀 수 있습니다. 메뉴 위 <b>검색창</b>에 이름을 입력하거나 방향키 + Enter로 빠르게 고르세요. (기존 설정의 ‘변환’과 텍스트 선택 시 ‘Convert to’는 <b>전환</b>으로 통합했습니다.)' } },
      { type: 'list', data: { style: 'unordered', items: [
        { content: '<b>여러 블록 일괄</b>: 본문을 드래그해 여러 블록을 선택한 뒤 전환하면 한꺼번에 바뀝니다. 전환할 수 없는 <b>표·이미지 등은 그대로</b> 유지돼요.', items: [] },
        { content: '<b>목록/토글 → 텍스트·체크리스트</b>: 묶여 있던 줄들이 <b>각각의 항목으로 분할</b>됩니다.', items: [] },
        { content: '<b>여러 텍스트 → 목록/토글</b>: <b>하나의 목록 항목들</b>로(또는 개별 토글로) 묶입니다.', items: [] },
      ] } },

      { type: 'header', data: { text: '➕ 3. 블록 추가 ( + 또는 / )', level: 2 } },
      ...
      { type: 'header', data: { text: '🧮 4. 표와 함수', level: 2 } },
      ...
      { type: 'header', data: { text: '↕️ 5. 순서 바꾸기 (드래그)', level: 2 } },
      ...
      { type: 'header', data: { text: '🗂️ 6. 하위 페이지 (본문 ↔ 좌측 탭 자동 연동)', level: 2 } },
      { type: 'list', data: { style: 'unordered', items: [
      ...
      { type: 'checklist', data: { items: [
        ...
        { text: '잘못 고쳤다면 상단 <b>← 되돌리기</b>(Ctrl+Z), 다시 <b>→</b>(Ctrl+Shift+Z)', checked: true },
        { text: '구글 모드면 내 드라이브의 <code class="inline-code">DARAKBANG</code> 폴더에 저장', checked: false },
      ] } },

      { type: 'header', data: { text: '🛠️ 9. 페이지 관리 / 내보내기', level: 2 } },
      ...
      { type: 'header', data: { text: '🔎 10. 검색 & ⭐ 즐겨찾기', level: 2 } },
      { type: 'paragraph', data: { text: '사이드바 상단 검색창은 <b>제목과 본문 내용</b>을 함께 찾습니다. 자주 보는 페이지는 “⋯ → 즐겨찾기”로 위쪽에 고정하세요.' } },

      { type: 'header', data: { text: '☁️ 11. 구글 드라이브로 저장 (배포용)', level: 2 } },
      ...
      { type: 'header', data: { text: '🔐 12. 로그인 / 데모', level: 2 } },
      { type: 'list', data: { style: 'unordered', items: [
